...
### Phase 4: Callback-aware frame pacing — 2026-08-05 to 2026-08-14
...
- **Mixed-output/mixed-refresh hardware gate:** A second physical HDMI output closed the remaining external gate. The exact installed binary (SHA-256 `ab3c466bdf890cbc8c791b4f999ead1004d19c963557fc6ce8a0339eb59d8b99`) ran the GPU `glass_drift` path with EGL 1.5 / GLES 2.0 while `eDP-1` was 1920x1200@60.026 and `HDMI-A-1` was temporarily 1280x720@50.000. `WAYLAND_DEBUG=client` mapped default-queue frame callbacks to the two physical `wl_surface`s. In a 10.055995022-second 15 FPS control window, both outputs completed 151 callbacks (15.016 callbacks/s) with none pending. In a 10.016898350-second 240 FPS window, eDP completed 600 callbacks (59.899/s) and HDMI completed 500 (49.916/s), with one end-boundary callback pending on each surface. The faster output therefore followed its own compositor cadence instead of being capped at the slower output's 50 Hz. Protocol logging adds observer overhead, and these are callback counts rather than presentation timestamps, GPU-power measurements, or a cross-compositor result. Machine-inspected 1920x1200 and 1280x720 captures (SHA-256 `d1dacf89f45ebe9fae9aced964edfa1abdb6960bc8a122ae5adcc401109a7d64` and `232c9f5a67f85c7d8f558c7c707503bd25eba2d8803ef9410a91f5c71e2436af`) showed both outputs populated with the wallpaper; no human visual-quality claim is made.
- **Final restoration and remote gates:** The temporary daemon exited through `wlchroma-ctl stop`; HDMI returned to its preferred 1280x720@60.000 mode. The normal daemon was restored as PID `85982` with the same installed binary hash, original config hash `ad4e6b1f9360e1a81879f5ba0d53a2b8608bcf8d0cae067ec14c7d7fde17118c`, and `colormix`/24 FPS/0.20/custom query; `mpris-chroma.service` was active as PID `86017`. The detailed report remains local ignored evidence at `.superpowers/sdd/phase4-live-verification.md`. On final code commit `7550634`, GitHub Actions Lint run `31708945616` and Test run `31708945583` both completed successfully; the Test job built the project and passed the full suite. The later documentation-only Phase 4 closeout at `0a9661a` also passed Lint run `31709086737` and Test run `31709086657`.
